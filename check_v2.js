                ...
                async function applyModeUI(fullSub) {
                    ...
                    if (fullSub) {
                        ...
                        if (pc) { pc.value = 'フル昇華'; pc.style.display = 'block' }
            ...
            <tr>
                ...
                function openProcSidebar() {
                    try {
                        ...
                        console.error("openProcSidebar error:", e); 
                        alert("加工パネルの起動エラー: " + e.message);
                    }
                }
                ...
                function openItemInputSidebar(forCarryIn = false) {
                    ...
                    if (sbPattern) {
                        const allowManualSku = forCarryIn || isFullSubMode;
                        if (allowManualSku) {
                    ...
                    // サザンラガー（またはフル昇華）向けの候補リストを構築
                    const slList = document.getElementById('itemSuggestionList');
                    if (slList) {
                        slList.innerHTML = "";
                        if (isSL) {
                            Object.keys(SOUTHERN_LAGER_MASTER).forEach(sku => {
                                const item = SOUTHERN_LAGER_MASTER[sku];
                                const opt = document.createElement('option');
                                opt.value = `${item.no} / ${item.name}`;
                                slList.appendChild(opt);
                            });
                        }
                    }
                ...
                window.autoFillPatternNo = function() {
                    const input = document.getElementById('sb_name').value.trim();
                    const sbPattern = document.getElementById('sb_pattern');
                    const sbProdName = document.getElementById('sb_product_name');
                    if (!input || !sbPattern) return;

                    const resolved = resolveProductInfo(input);
                    if (resolved && resolved.price > 0) {
                        sbPattern.value = resolved.no;
                        if(sbProdName) sbProdName.value = resolved.name;
                        syncDraftData();
                    }
                }
